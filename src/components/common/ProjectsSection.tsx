...
const ProjectsSection: React.FC<ProjectsSectionProps> = ({ projects }) => {
  ...
  // Mock data for now
  const stats = [
    { value: tStats('stat1.value'), description: tStats('stat1.description') },
    { value: tStats('stat2.value'), description: tStats('stat2.description') },
    { value: tStats('stat3.value'), description: tStats('stat3.description') },
  ];
        ...
        <div className="space-y-10">
          {stats.map((stat, index) => (
            <div key={index} className="border-b border-gray-200 pb-1">
              <p className="text-7xl font-normal font-cinzel text-[#1976d2]">{stat.value}</p>
