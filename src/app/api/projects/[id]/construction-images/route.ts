     ...
     async index(request: NextRequest, params: { id: string }) {
          try {
               ...
               const constructionImages =
                    await ProjectConstructionImage.findAll({
                         where: { projectId },
                         order: [['createdAt', 'ASC']],
                    });
               ...
               return this.success(
                    constructionImages,
                    'İnşaat resimleri başarıyla getirildi'
               );
          } catch (error: unknown) {
               console.error('Construction images index error:', error);
               return this.error(
                    'İnşaat resimleri getirilirken hata oluştu',
                    500
               );
          }
     }
               ...
               if (contentType.includes('multipart/form-data')) {
                    ...
                    try {
                         ...
                         const constructionImage =
                              await ProjectConstructionImage.create({
                                   projectId,
                                   imageUrl,
                              });
                         ...
                         return this.success(
                              constructionImage,
                              'İnşaat resmi başarıyla eklendi',
                              201
                         );
                    } catch (uploadError) {
