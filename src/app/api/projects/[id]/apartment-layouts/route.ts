...
export class ApartmentLayoutController extends BaseController {
  ...
  async index(request: NextRequest, params: { id: string }) {
    try {
      ...
      return this.success(apartmentLayouts, 'Daire planları başarıyla getirildi');
    } catch (error: unknown) {
      console.error('Apartment layouts index error:', error);
      return this.error('Daire planları getirilirken hata oluştu', 500);
    }
  }
  ...
  async store(request: NextRequest, params: { id: string }) {
    try {
      ...
      if (contentType.includes('multipart/form-data')) {
        ...
        const layoutImageFile = formData.get('layoutImage') as File | null;
        ...
        if (!totalArea || isNaN(parseFloat(totalArea))) {
          return this.error('Toplam alan geçerli bir sayı olmalıdır', 400);
        }

        try {
          const layoutImageUrl = await uploadFileToServer(layoutImageFile);
          ...
          const apartmentLayout = await ProjectApartmentLayout.create({
            ...
            kitchenArea: kitchenArea ? parseFloat(kitchenArea) : undefined,
            bathroomArea: bathroomArea ? parseFloat(bathroomArea) : undefined,
            balconyArea: balconyArea ? parseFloat(balconyArea) : undefined
          });

          return this.success(apartmentLayout, 'Daire planı başarıyla eklendi', 201);
        } catch (uploadError) {
          console.error('Layout image upload error:', uploadError);
          return this.error('Plan resmi yüklenirken hata oluştu', 400);
        }
      } else if (contentType.includes('application/json')) {
        const body = await this.parseBody(request) as Record<string, unknown>;
        
        // Validate required fields
        if (!body.apartmentType || typeof body.apartmentType !== 'string') {
          return this.error('Daire tipi gereklidir', 400);
        }

        if (!body.layoutImageUrl || typeof body.layoutImageUrl !== 'string') {
        ...
        const apartmentLayout = await ProjectApartmentLayout.create({
          projectId,
          apartmentType: (body.apartmentType as string).trim(),
          squareMeter: body.squareMeter ? (body.squareMeter as string).trim() : '',
          layoutImageUrl: body.layoutImageUrl as string,
          totalArea: body.totalArea as number,
          roomArea: body.roomArea ? (body.roomArea as number) : undefined,
          kitchenArea: body.kitchenArea ? (body.kitchenArea as number) : undefined,
          bathroomArea: body.bathroomArea ? (body.bathroomArea as number) : undefined,
          balconyArea: body.balconyArea ? (body.balconyArea as number) : undefined
        });
