...
export class ProjectController extends BaseController {
    ...
    try {
      ...
      if (contentType.includes("multipart/form-data")) {
        ...
        if (headerImageFile && headerImageFile.size > 0) {
          try {
            headerImageUrl = await uploadFileToServer(headerImageFile);
          } catch (uploadError) {
            console.error("Header image upload error:", uploadError);
            return this.error("Başlık resmi yüklenirken hata oluştu", 400);
          }
        }
        ...
        if (catalogFile && catalogFile.size > 0) {
          try {
            catalogUrl = await uploadFileToServer(catalogFile);
          } catch (uploadError) {
            console.error("Catalog upload error:", uploadError);
            return this.error("Katalog yüklenirken hata oluştu", 400);
          }
        }
        ...
        if (featureImageFile && featureImageFile.size > 0) {
          try {
            featureImageUrl = await uploadFileToServer(featureImageFile);
          } catch (uploadError) {
            console.error("Feature image upload error:", uploadError);
            return this.error("Özellik resmi yüklenirken hata oluştu", 400);
          }
        }
        ...
        if (generalPlanImageFile && generalPlanImageFile.size > 0) {
          try {
            generalPlanImageUrl = await uploadFileToServer(
              generalPlanImageFile
            );
          } catch (uploadError) {
            console.error("General plan image upload error:", uploadError);
            return this.error("Genel plan resmi yüklenirken hata oluştu", 400);
          }
        }
        ...
        const interiorImages: Array<{ imageUrl: string; caption?: string }> =
          [];
        const interiorImageFiles = formData.getAll("interiorImages") as File[];
        ...
        for (let i = 0; i < interiorImageFiles.length; i++) {
          ...
          if (file && file.size > 0) {
            try {
              ...
              console.error("Interior image upload error:", uploadError);
              return this.error(
        ...
        for (let i = 0; i < constructionImageFiles.length; i++) {
          ...
          if (file && file.size > 0) {
            try {
              ...
              console.error("Construction image upload error:", uploadError);
              return this.error(
        ...
        const apartmentTypes = formData.getAll("apartmentTypes") as string[];
        ...
        if (apartmentTypes.length > 0) {
          ...
          for (let i = 0; i < apartmentTypes.length; i++) {
            ...
            if (
              ...
              try {
                layoutImageUrl = await uploadFileToServer(
                  apartmentLayoutFiles[i]
                );
              } catch (uploadError) {
                console.error(
            ...
            if (apartmentTypes[i]) {
              apartments.push({
                ...
                totalArea: apartmentLayoutTotalAreas[i]
                  ? parseFloat(apartmentLayoutTotalAreas[i])
                  : 0,
                roomArea: apartmentLayoutRoomAreas[i]
                  ? parseFloat(apartmentLayoutRoomAreas[i])
                  : undefined,
                kitchenArea: apartmentLayoutKitchenAreas[i]
                  ? parseFloat(apartmentLayoutKitchenAreas[i])
                  : undefined,
                bathroomArea: apartmentLayoutBathroomAreas[i]
                  ? parseFloat(apartmentLayoutBathroomAreas[i])
                  : undefined,
                balconyArea: apartmentLayoutBalconyAreas[i]
                  ? parseFloat(apartmentLayoutBalconyAreas[i])
                  : undefined,
              });
            }
          }
        ...
        projectData = {
          ...
          constructionImages:
            (body.constructionImages as Array<{ imageUrl: string }>)?.map(
              (img) => ({
                imageUrl: img.imageUrl,
              })
            ) || [],
  ...
  async update(request: NextRequest, params: { id: string }) {
    try {
      ...
      if (contentType.includes("multipart/form-data")) {
        ...
        let generalPlanImageUrl: string | undefined | null = undefined;
        ...
        projectData = {
          ...
          ...(status !== undefined && { status: status?.trim() || undefined }),
          ...(year !== undefined && {
            year: year ? parseInt(year) : undefined,
          }),
          ...(area !== undefined && { area: area?.trim() || undefined }),
          ...(block !== undefined && {
            block: block ? parseInt(block) : undefined,
          }),
          // Yeni eklenen ilişkisel alanlar
          ...
          ...(generalPlanImageUrl !== undefined && { generalPlanImageUrl }),
        };
        ...
        if (interiorImageFiles.length > 0 || formData.has("interiorImages")) {
          const interiorImages: Array<{ imageUrl: string; caption?: string }> =
            [];
        ...
        projectData = {
          ...
          ...(body.year !== undefined && { year: body.year as number }),
          ...(body.area !== undefined && { area: body.area as string }),
          ...(body.block !== undefined && { block: body.block as number }),
  ...
  private validateCreateProject(
    ...
    if (data.location && typeof data.location !== "string") {
      errors.push({ field: "location", message: "Konum metin olmalıdır" });
    }

    if (data.status && typeof data.status !== "string") {
      errors.push({ field: "status", message: "Durum metin olmalıdır" });
    }
    ...
    if (
      ...
      errors.push({ field: "block", message: "Blok sayısı sayı olmalıdır" });
    }
