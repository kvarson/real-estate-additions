...
export const createDatabaseIfNotExists = async () => {
     try {
          ...
          console.log(
               '❌ Belirtilen veritabanına bağlantı kurulamadı, veritabanı oluşturuluyor...'
          );

          // If connection fails, try to create the database
          const sequelizeAdmin = new Sequelize(
               `postgres://${dbUser}:${dbPassword}@${dbHost}:${dbPort}/postgres`,
               {
                    dialect: 'postgres',
                    logging: false,
                    dialectOptions: {
...
export const connectDB = async () => {
     try {
          ...
          if (
               ...
               sequelizeError.original &&
               typeof sequelizeError.original === 'object' &&
               sequelizeError.original !== null &&
               'code' in sequelizeError.original &&
               (sequelizeError.original as Record<string, unknown>).code ===
                    '3D000'
          ) {
               console.error(
                    '❌ Database bulunamadı. Lütfen veritabanının varlığını kontrol edin.'
               );
          }
