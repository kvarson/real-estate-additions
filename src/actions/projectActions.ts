     ...
     try {
          ...
          if (image && image.size > 0) {
               ...
               if (!allowedTypes.includes(image.type)) {
                    throw new Error(
                         'Sadece JPG, PNG, GIF veya WEBP formatında resim yükleyebilirsiniz'
                    );
               }
          ...
          redirect(
               `/az/projects/create?error=${encodeURIComponent(errorMessage)}`
          );
     }
}
...
export async function updateProjectAction(id: number, formData: FormData) {
     try {
          ...
          redirect(
               `/az/projects/${id}/edit?error=${encodeURIComponent(errorMessage)}`
          );
     }
}
